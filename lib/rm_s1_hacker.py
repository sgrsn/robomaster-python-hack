...
class RoboMasterHacker:
    ...
    if received_msg is None:
      ...
      if id == 0x201:
        ...
        if recv_data[0:6] == [0x55, 0x1b, 0x04, 0x75, 0x09, 0xc3]:
          self.joy_cmd_counter = (recv_data[6] | (recv_data[7]<<8) ) + 1

  def send_touch_command(self):
    start_time_touch = time.time()
    touch_msg_list = [[0x55, 0x0f, 0x04, 0xa2, 0x09, 0x04, self.joy_cmd_counter&0xFF, (self.joy_cmd_counter>>8)&0xFF],
    ...
    self.joy_cmd_counter+=1
    
  def send_joy_command(self, vx, vy, z):
    joy_msg_list = [[0x55, 0x1b, 0x04, 0x75, 0x09, 0xc3, self.joy_cmd_counter&0xFF, (self.joy_cmd_counter>>8)&0xFF],  #0-7
                  [0x00, 0x3f, 0x60,                  #8-10
                  vy&0xFF,                            #11
                  ((vx<<3) | ((vy>>8)&0x07)) & 0xFF,  #12
                  (0x20 & 0xC0) | ((vx>>5)&0x3F),     #13
                  0x00, 0x01],                        #14, 15
                  [(z<<4 | 0x08)&0xFF,                #16
                  (z>>4)&0xFF,                        #17
                  0x00,                               #18
                  ...
                  0x00],                              #23
                  [0x04]]                             #24
    ucCRC16 = self.getCRC16CheckSum(joy_msg_list[0]+joy_msg_list[1]+joy_msg_list[2]+joy_msg_list[3], CRC16_INIT)
    joy_msg_list[3].append( ucCRC16 & 0xFF )
    joy_msg_list[3].append( (ucCRC16>>8) & 0xFF )
    self.send_msg(joy_msg_list)
    self.joy_cmd_counter+=1

  def send_led_command(self, r, g, b):
    led_msg_list = [[0x55, 0x1a, 0x04, 0xb1, 0x09, 0x18, self.led_cmd_counter&0xFF, (self.led_cmd_counter>>8)&0xFF],
                  [0x00, 0x3f, 0x32, 0x01, 0xff, 0x00, r, g],
                  [b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x3f, 0x00]]
    send_msg(led_msg_list)
  ...
  def appendCRC8CheckSum(self, pchMessage):
    ucCRC = 0
    ucCRC = getCRC8CheckSum(pchMessage, CRC8_INIT)
    pchMessage.append(ucCRC)

  def appendCRC16CheckSum(self, pchMessage):
    ucCRC = 0
    ucCRC = getCRC16CheckSum(pchMessage, CRC16_INIT)
    ...
    while True:
      if kbhit():
          ...
          if key == 'd':
            ...
            z = 1024
      hacker.receive_msg()
      if time.time() - start_time_touch > send_interval_touch:
        hacker.send_touch_command()
      if time.time() - start_time > send_interval:
        hacker.send_joy_command(vx, vy, z)
